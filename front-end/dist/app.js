(()=>{var R=class extends HTMLElement{connectedCallback(){this.render=this.render.bind(this),this.updateActive=this.updateActive.bind(this),this.handleDocClick=this.handleDocClick.bind(this),this.handleUserUpdated=this.handleUserUpdated.bind(this),this.handleAppStateChanged=this.handleAppStateChanged.bind(this),this.loadUser=this.loadUser.bind(this),this.scheduleIdle=this.scheduleIdle?.bind(this)||(e=>{typeof window.requestIdleCallback=="function"?window.requestIdleCallback(()=>e(),{timeout:1200}):setTimeout(e,300)}),this.render(),window.addEventListener("hashchange",this.updateActive),window.addEventListener("user-updated",this.handleUserUpdated),window.addEventListener("app-state-changed",this.handleAppStateChanged),this.scheduleIdle(this.loadUser),this.updateActive()}disconnectedCallback(){window.removeEventListener("hashchange",this.updateActive),window.removeEventListener("user-updated",this.handleUserUpdated),window.removeEventListener("app-state-changed",this.handleAppStateChanged),document.removeEventListener("click",this.handleDocClick)}getAppState(){let e=localStorage.getItem("appState");if(e==="customer"||e==="courier")return e;let t=localStorage.getItem("authRole");return t==="customer"||t==="courier"?t:"guest"}getNavLinks(){let e=this.getAppState(),t=[];return e==="customer"&&(t.push({href:"#home",label:"\u041D\u04AF\u04AF\u0440",icon:"home"}),t.push({href:"#orders",label:"\u0417\u0430\u0445\u0438\u0430\u043B\u0433\u0430",icon:"orders"})),e==="courier"&&(t.push({href:"#home",label:"\u041D\u04AF\u04AF\u0440",icon:"home"}),t.push({href:"#delivery",label:"\u0425\u04AF\u0440\u0433\u044D\u043B\u0442",icon:"delivery"})),e!=="guest"&&t.push({href:"#profile",label:"\u041F\u0440\u043E\u0444\u0430\u0439\u043B",icon:"profile"}),t}renderNavLinks(){return this.getNavLinks().map(t=>`
    ...
    `}applyActiveOrder(){this.fetchActiveOrder()}async fetchActiveOrder(){try{let e=await fetch("/api/active-order");if(!e.ok)return;let r=(await e.json())?.order;if(!r)return;let i=this.querySelector('d-orders[data-active="true"]');i&&r.from&&r.to&&(i.setAttribute("header",`${r.from} \u2192 ${r.to}`),r.item&&i.setAttribute("detail",r.item))}catch{}}renderDeliveryCart(){let e=this.querySelector("#deliveryList");e&&this.fetchDeliveryItems(e)}attachOrderSelection(e=[]){let t=this.querySelectorAll("d-orders");if(!t.length)return;let r=new Map(e.map(n=>[String(n.id||""),n]));t.forEach(n=>{n.addEventListener("click",()=>{let d=n.getAttribute("data-id"),l=r.get(String(d))||null;this.selectOrder(l,n)})});let i=t[0],s=i?.getAttribute("data-id"),a=r.get(String(s))||null;a&&this.selectOrder(a,i)}selectOrder(e,t){if(!e)return;this.querySelectorAll("d-orders").forEach(i=>{i.classList.toggle("is-active",i===t)});let r=new CustomEvent("delivery-select",{detail:{...e,id:e.orderId||e.id||null,orderId:e.orderId||null}});document.dispatchEvent(r)}escapeAttr(e){return String(e||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}async fetchDeliveryItems(e){let t=[];try{let a=await fetch("/api/delivery-cart");if(a.ok){let n=await a.json();t=Array.isArray(n?.items)?n.items:[]}}catch{t=[]}if(!t.length){e.innerHTML='<p class="muted">\u0425\u04AF\u0440\u0433\u044D\u043B\u0442 \u0441\u043E\u043D\u0433\u043E\u043E\u0433\u04AF\u0439 \u0431\u0430\u0439\u043D\u0430.</p>';let a=this.querySelector("del-order-details"),n=this.querySelector("del-order-progress");a&&a.setAttribute("data-empty","1"),n&&n.setAttribute("data-empty","1");return}let r=this.querySelector("del-order-details"),i=this.querySelector("del-order-progress");r&&r.removeAttribute("data-empty"),i&&i.removeAttribute("data-empty");let s=t.map(a=>{let n=Number(a.qty||1),d=[];a.meta&&d.push(a.meta),console.log(a?.menuItemName),n&&d.push(`x${n}`);let l=d.join(" \u2022 "),c=String(a.title||""),u=c.split(" - "),h=u[0]||c,y=u.slice(1).join(" - "),b=a.order_id||null;return{...a,detail:l,from:h,to:y,createdAt:a.meta||"",orderId:b}});e.innerHTML=s.map(a=>`
      ...
      `).join(""),this.attachOrderSelection(s)}};customElements.define("delivery-page",H);var N=class extends HTMLElement{connectedCallback(){this._initialized=!1,this.handleRouteChange=this.handleRouteChange?.bind(this)||(()=>this.onRouteChange()),window.addEventListener("hashchange",this.handleRouteChange),this.handleRouteChange()}onRouteChange(){if(location.hash!=="#orders"){this.orderStream&&(this.handleOrderStatusEvent&&this.orderStream.removeEventListener("order-status",this.handleOrderStatusEvent),this.handleOrderUpdatedEvent&&this.orderStream.removeEventListener("order-updated",this.handleOrderUpdatedEvent),this.orderStream.close(),this.orderStream=null);return}this.selectedOrder=null,this._onDocClick=this._onDocClick?.bind(this)||(e=>this.onDocClick(e)),this._initialized||(this.render(),this.bindModal(),this.bindClicks(),this.bindProgress(),this._initialized=!0),this.loadOrders(),this.initOrderStream()}disconnectedCallback(){window.removeEventListener("hashchange",this.handleRouteChange),document.removeEventListener("click",this._onDocClick),this.orderStream&&(this.handleOrderStatusEvent&&this.orderStream.removeEventListener("order-status",this.handleOrderStatusEvent),this.handleOrderUpdatedEvent&&this.orderStream.removeEventListener("order-updated",this.handleOrderUpdatedEvent),this.orderStream.close(),this.orderStream=null)}renderCourier(e){let t=this.querySelector("#courierBox");if(t){if(!e){t.setEmpty?.();return}t.setData?.(e)}}async loadCourierForOrder(e){let t=this.querySelector("#courierBox");if(!t)return;if(!e){t.setEmpty?.();return}let r=e?.courier||null;if(!r){t.setEmpty?.();return}try{let i=await m("/api/auth/me");if(i.ok){let{user:s}=await i.json();if(console.log("\u041E\u0434\u043E\u043E\u0433\u0438\u0439\u043D \u0445\u044D\u0440\u044D\u0433\u043B\u044D\u0433\u0447:",s),console.log("\u0417\u0430\u0445\u0438\u0430\u043B\u0433\u044B\u043D \u0445\u04AF\u0440\u0433\u044D\u0433\u0447:",r),s&&r&&String(s.id)===String(r.id)){t.setEmpty?.();return}}}catch{}this.renderCourier(r)}render(){this.innerHTML=`
    ...
    `;let l=this.querySelector(".btn-close"),c=this.querySelector(".btn-pay"),u=this.querySelector(".btn-go"),h=this.querySelector(".status"),y=this.querySelectorAll('input[name="paymentMethod"]'),b=()=>{h&&(h.hidden=!1),c&&(c.disabled=!0,c.textContent="\u0422\u04E9\u043B\u0431\u04E9\u0440 \u0442\u04E9\u043B\u04E9\u0433\u0434\u0441\u04E9\u043D"),u&&(u.hidden=!1)},O=()=>{h&&(h.hidden=!0),c&&(c.disabled=!1,c.textContent="\u0422\u04E9\u043B\u0431\u04E9\u0440 \u0442\u04E9\u043B\u04E9\u0445"),u&&(u.hidden=!0)},v=()=>{let p=this.querySelector('input[name="paymentMethod"]:checked')?.value||"card",f=this.querySelector(".payment-details--card"),g=this.querySelector(".payment-details--qpay");f&&(f.hidden=p!=="card"),g&&(g.hidden=p!=="qpay")};l?.addEventListener("click",()=>location.hash="#profile"),y.forEach(p=>p.addEventListener("change",v)),v(),localStorage.setItem("courierPaid",d()?"1":"0"),d()?b():O(),c?.addEventListener("click",()=>{localStorage.setItem(n,"1"),localStorage.setItem("courierPaid","1"),b(),window.dispatchEvent(new Event("user-updated"))}),u?.addEventListener("click",()=>{location.hash="#delivery"})}};customElements.define("pay-page",P);var B=class extends HTMLElement{connectedCallback(){this.currentMode="login",this.currentRole="customer",this.innerHTML=`
  ...
  <div class="scene">
    ...
    `}events(){this.addEventListener("click",()=>{let e=this.data;this.dispatchEvent(new CustomEvent("offer-select",{bubbles:!0,detail:e}))}),this.addEventListener("keydown",e=>{(e.key==="Enter"||e.key===" ")&&(e.preventDefault(),this.click())})}};customElements.get("offer-card")||customElements.define("offer-card",J);function C(o){return String(o??"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}var V=class extends HTMLElement{connectedCallback(){this._ready||(this._ready=!0,this._loaded=!1,this.handleRouteChange=this.handleRouteChange?.bind(this)||(()=>this.onRouteChange()),this.render(),this.handleOffersUpdated=()=>pe(),window.addEventListener("offers-updated",this.handleOffersUpdated),window.addEventListener("hashchange",this.handleRouteChange),this.addEventListener("offer-select",this.handleOfferSelect),this.handleRouteChange())}disconnectedCallback(){this.handleOffersUpdated&&window.removeEventListener("offers-updated",this.handleOffersUpdated),window.removeEventListener("hashchange",this.handleRouteChange),this.removeEventListener("offer-select",this.handleOfferSelect)}render(){this.innerHTML=`
      <section class="offers-container">
        <div class="offers-row"></div>
      </section>
    `}set items(e){this.renderItems(e)}renderItems(e){let t=this.querySelector(".offers-row");t.innerHTML=e.map(Le).join("")}handleOfferSelect(e){let t=document.querySelector("offer-modal");t&&t.show(e.detail)}onRouteChange(){if(location.hash!=="#home"&&location.hash!==""||this._loaded===!0)return;this._loaded=!0;let e=$e();this.renderItems(e),pe()}};customElements.get("offers-list")||customElements.define("offers-list",V);var k={single:"assets/img/document.svg",medium:"assets/img/tor.svg",large:"assets/img/box.svg"};function Le(o){let e=o.thumb||"assets/img/document.svg",t=o.title||"",r=o.meta||"",i=o.price||"",s=o.orderId||o.id||"",a=C(JSON.stringify(o.sub||[])),n=C(JSON.stringify(o.customer||{}));return`
    <offer-card
      thumb="${e}"
      title="${t}"
      meta="${r}"
  ...
  `}function fe(o){try{let e=String(o).split("\u2022"),[t,r]=e,[i,s,a]=t.split("/"),[n,d]=r.split(":"),l=2e3+Number.parseInt(a,10);return new Date(l,Number(i)-1,Number(s),Number(n),Number(d))}catch(e){console.error("date error shu:",e,o)}}function ge(o){let e=o?.scheduled_at;if(!e)return null;let t=Date.parse(e);if(!isNaN(t))return t;let r=fe(e);return r?r.getTime():null}function xe(o){let e=ge(o);return e?e<Date.now():!0}function ke(o){return o>5?k.large:o>=2?k.medium:o===1?k.single:k.large}function Me(o){let e=o?.from_name||"",t=o?.to_name||"";return[e,t].filter(r=>r!=="").join(" - ")}function Ae(o){return o.filter(t=>{let r=t?.status;return r==="delivered"||r==="cancelled"||t?.courier?!1:!xe(t)}).map(t=>{let r=t?.items,i=r.reduce((n,d)=>n+(d?.qty??0),0),s=r.map(n=>({name:`${n?.name} x${n?.qty}`})),a=ge(t);return{orderId:t?.id||"",title:Me(t),meta:x(a),price:w(t?.total_amount),thumb:ke(i),customer:t?.customer,sub:s}})}async function qe(){let o=await m("/api/orders");if(!o.ok)return[];let e=await o.json();return Ae(e)}function $e(){let o=localStorage.getItem("offers");if(!o)return[];try{return JSON.parse(o)||[]}catch{return[]}}function Re(o){let e=localStorage.getItem("authUserKey");return e?`${o}:${e}`:o}function Te(){let o=localStorage.getItem(Re("removed_offer_ids"));if(!o)return[];try{return JSON.parse(o)||[]}catch{return[]}}function _e(o){let e=Te();return e.length?o.filter(t=>{let r=t?.orderId??t?.id??null;return r?!e.includes(String(r)):!0}):o}function De(o){return o.filter(e=>{if(!e?.meta)return!0;let t=fe(e.meta);return t?t.getTime()>=Date.now():!0})}async function pe(){let o=[];try{o=await qe()}catch(e){console.error("Offers API-\u0430\u0430\u0441 \u0430\u0432\u0430\u0445\u0430\u0434 \u0430\u043B\u0434\u0430\u0430:",e),o=[]}o=De(o),o=_e(o),localStorage.setItem("offers",JSON.stringify(o)),document.querySelectorAll("offers-list").forEach(e=>{e.items=o})}var z=class extends HTMLElement{connectedCallback(){this.scheduleIdle=this.scheduleIdle?.bind(this)||(e=>{typeof window.requestIdleCallback=="function"?window.requestIdleCallback(()=>e(),{timeout:1200}):setTimeout(e,300)}),this.handleUpdate=this.handleUpdate.bind(this),this.handleClick=this.handleClick.bind(this),this.handleViewportChange=this.handleViewportChange.bind(this),this._loaded=!1,this.handleRouteChange=this.handleRouteChange?.bind(this)||(()=>this.onRouteChange()),this.render(),this.cacheEls(),this.addEventListener("click",this.handleClick),window.addEventListener("delivery-cart-updated",this.handleUpdate),window.addEventListener("hashchange",this.handleRouteChange),this.mediaQuery=window.matchMedia("(max-width: 900px)"),this.mediaQuery.addEventListener("change",this.handleViewportChange),this.handleViewportChange(),this.handleRouteChange()}disconnectedCallback(){window.removeEventListener("delivery-cart-updated",this.handleUpdate),window.removeEventListener("hashchange",this.handleRouteChange),this.removeEventListener("click",this.handleClick),this.mediaQuery&&this.mediaQuery.removeEventListener("change",this.handleViewportChange)}onRouteChange(){(location.hash||"#home")==="#home"&&(this._loaded||(this._loaded=!0,this.scheduleIdle(()=>this.load())))}render(){this.innerHTML=`
  ...
  <g id="Complete">
    ...
    `}};customElements.define("courier-card",te);var re=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.value=0}connectedCallback(){let e=parseInt(this.getAttribute("max"))||5,t=this.getAttribute("color")||"gold",r=this.getAttribute("size")||"26px",i=document.createElement("style");i.textContent=`
      .rating {
        display: flex;
        gap: 5px;
        cursor: pointer;
