(()=>{var R=class extends HTMLElement{connectedCallback(){this.render=this.render.bind(this),this.updateActive=this.updateActive.bind(this),this.handleDocClick=this.handleDocClick.bind(this),this.handleUserUpdated=this.handleUserUpdated.bind(this),this.handleAppStateChanged=this.handleAppStateChanged.bind(this),this.loadUser=this.loadUser.bind(this),this.scheduleIdle=this.scheduleIdle?.bind(this)||(e=>{typeof window.requestIdleCallback=="function"?window.requestIdleCallback(()=>e(),{timeout:1200}):setTimeout(e,300)}),this.render(),window.addEventListener("hashchange",this.updateActive),window.addEventListener("user-updated",this.handleUserUpdated),window.addEventListener("app-state-changed",this.handleAppStateChanged),this.scheduleIdle(this.loadUser),this.updateActive()}disconnectedCallback(){window.removeEventListener("hashchange",this.updateActive),window.removeEventListener("user-updated",this.handleUserUpdated),window.removeEventListener("app-state-changed",this.handleAppStateChanged),document.removeEventListener("click",this.handleDocClick)}getAppState(){let e=localStorage.getItem("appState");if(e==="customer"||e==="courier")return e;let t=localStorage.getItem("authRole");return t==="customer"||t==="courier"?t:"guest"}getNavLinks(){let e=this.getAppState(),t=[];return e==="customer"&&(t.push({href:"#home",label:"\u041D\u04AF\u04AF\u0440",icon:"home"}),t.push({href:"#orders",label:"\u0417\u0430\u0445\u0438\u0430\u043B\u0433\u0430",icon:"orders"})),e==="courier"&&(t.push({href:"#home",label:"\u041D\u04AF\u04AF\u0440",icon:"home"}),t.push({href:"#delivery",label:"\u0425\u04AF\u0440\u0433\u044D\u043B\u0442",icon:"delivery"})),e!=="guest"&&t.push({href:"#profile",label:"\u041F\u0440\u043E\u0444\u0430\u0439\u043B",icon:"profile"}),t}renderNavLinks(){return this.getNavLinks().map(t=>`
    ...
    `}parseOrderTimestamp(e){return this.parseDate(e.scheduled_at)}parseDate(e){let t=Date.parse(e);return Number.isNaN(t)?null:t}getOfferThumb(e){if(!e)return"";try{let t=localStorage.getItem("offers"),r=t?JSON.parse(t):[];return(Array.isArray(r)?r.find(s=>(s?.orderId||s?.id)===e):null)?.thumb||""}catch{return""}}};customElements.define("my-order",U);var B=class extends HTMLElement{connectedCallback(){this._initialized=!1,this.selectedOrder=null,this.handleRouteChange=this.handleRouteChange?.bind(this)||(()=>this.onRouteChange()),window.addEventListener("hashchange",this.handleRouteChange),this.handleRouteChange()}onRouteChange(){if(location.hash!=="#orders"){this.cleanup();return}this.selectedOrder=null,this._initialized||(this.render(),this.bindOrderCardClicks(),this._initialized=!0),this.loadOrders(),this.initOrderStream(),this.bindRatingComplete()}disconnectedCallback(){window.removeEventListener("hashchange",this.handleRouteChange),this.cleanup()}cleanup(){this.handleRatingComplete&&window.removeEventListener("rating-completed",this.handleRatingComplete),this.orderStream&&(this.handleOrderStatusEvent&&this.orderStream.removeEventListener("order-status",this.handleOrderStatusEvent),this.handleOrderUpdatedEvent&&this.orderStream.removeEventListener("order-updated",this.handleOrderUpdatedEvent),this.orderStream.close(),this.orderStream=null)}render(){this.innerHTML=`
    ...
    `;let l=this.querySelector(".btn-close"),c=this.querySelector(".btn-pay"),m=this.querySelector(".btn-go"),h=this.querySelector(".status"),b=this.querySelectorAll('input[name="paymentMethod"]'),w=()=>{h&&(h.hidden=!1),c&&(c.disabled=!0,c.textContent="\u0422\u04E9\u043B\u0431\u04E9\u0440 \u0442\u04E9\u043B\u04E9\u0433\u0434\u0441\u04E9\u043D"),m&&(m.hidden=!1)},k=()=>{h&&(h.hidden=!0),c&&(c.disabled=!1,c.textContent="\u0422\u04E9\u043B\u0431\u04E9\u0440 \u0442\u04E9\u043B\u04E9\u0445"),m&&(m.hidden=!0)},y=()=>{let p=this.querySelector('input[name="paymentMethod"]:checked')?.value||"card",f=this.querySelector(".payment-details--card"),g=this.querySelector(".payment-details--qpay");f&&(f.hidden=p!=="card"),g&&(g.hidden=p!=="qpay")};l?.addEventListener("click",()=>location.hash="#profile"),b.forEach(p=>p.addEventListener("change",y)),y(),localStorage.setItem("courierPaid",d()?"1":"0"),d()?w():k(),c?.addEventListener("click",()=>{localStorage.setItem(n,"1"),localStorage.setItem("courierPaid","1"),w(),window.dispatchEvent(new Event("user-updated"))}),m?.addEventListener("click",()=>{location.hash="#delivery"})}};customElements.define("pay-page",j);var F=class extends HTMLElement{connectedCallback(){this.currentMode="login",this.currentRole="customer",this.innerHTML=`
  ...
  <div class="scene">
    ...
    `}events(){this.addEventListener("click",()=>{let e=this.data;this.dispatchEvent(new CustomEvent("offer-select",{bubbles:!0,detail:e}))}),this.addEventListener("keydown",e=>{(e.key==="Enter"||e.key===" ")&&(e.preventDefault(),this.click())})}};customElements.get("offer-card")||customElements.define("offer-card",V);var Z=class extends HTMLElement{connectedCallback(){this._ready||(this._ready=!0,this._loaded=!1,this.handleRouteChange=this.handleRouteChange?.bind(this)||(()=>this.onRouteChange()),this.render(),this.handleOffersUpdated=()=>Se(),window.addEventListener("offers-updated",this.handleOffersUpdated),window.addEventListener("hashchange",this.handleRouteChange),this.addEventListener("offer-select",this.handleOfferSelect),this.handleRouteChange())}disconnectedCallback(){this.handleOffersUpdated&&window.removeEventListener("offers-updated",this.handleOffersUpdated),window.removeEventListener("hashchange",this.handleRouteChange),this.removeEventListener("offer-select",this.handleOfferSelect)}render(){this.innerHTML=`
      <section class="offers-container">
        <div class="offers-row"></div>
      </section>
    `}set items(e){this.renderItems(e)}renderItems(e){let t=this.querySelector(".offers-row");t.innerHTML=e.map($e).join("")}handleOfferSelect(e){let t=document.querySelector("offer-modal");t&&t.show(e.detail)}onRouteChange(){if(location.hash!=="#home"&&location.hash!==""||this._loaded===!0)return;this._loaded=!0;let e=De();this.renderItems(e),Se()}};customElements.get("offers-list")||customElements.define("offers-list",Z);function $e(o){let e=o.thumb||"assets/img/document.svg",t=o.title||"",r=o.meta||"",i=o.price||"",s=o.orderId||o.id||"",a=v(JSON.stringify(o.sub||[])),n=v(JSON.stringify(o.customer||{}));return`
    <offer-card
      thumb="${e}"
      title="${t}"
      meta="${r}"
  ...
  `}function we(o){try{let e=String(o).split("\u2022"),[t,r]=e,[i,s,a]=t.split("/"),[n,d]=r.split(":"),l=2e3+Number.parseInt(a,10);return new Date(l,Number(i)-1,Number(s),Number(n),Number(d))}catch(e){console.error("date error shu:",e,o)}}function Ce(o){let e=o?.scheduled_at;if(!e)return null;let t=Date.parse(e);if(!isNaN(t))return t;let r=we(e);return r?r.getTime():null}function Te(o){let e=Ce(o);return e?e<Date.now():!0}function Re(o){let e=o?.from_name||"",t=o?.to_name||"";return[e,t].filter(r=>r!=="").join(" - ")}function _e(o){return o.filter(t=>{let r=t?.status;return r==="delivered"||r==="cancelled"||t?.courier?!1:!Te(t)}).map(t=>{let r=t?.items,i=r.reduce((n,d)=>n+(d?.qty??0),0),s=r.map(n=>({name:`${n?.name} x${n?.qty}`})),a=Ce(t);return{orderId:t?.id||"",title:Re(t),meta:I(a),price:S(t?.total_amount),thumb:O(i),customer:t?.customer,sub:s}})}async function He(){let o=await u("/api/orders");if(!o.ok)return[];let e=await o.json();return _e(e)}function De(){let o=localStorage.getItem("offers");if(!o)return[];try{return JSON.parse(o)||[]}catch{return[]}}function Ne(o){let e=localStorage.getItem("authUserKey");return e?`${o}:${e}`:o}function Ue(){let o=localStorage.getItem(Ne("removed_offer_ids"));if(!o)return[];try{return JSON.parse(o)||[]}catch{return[]}}function Be(o){let e=Ue();return e.length?o.filter(t=>{let r=t?.orderId??t?.id??null;return r?!e.includes(String(r)):!0}):o}function Pe(o){return o.filter(e=>{if(!e?.meta)return!0;let t=we(e.meta);return t?t.getTime()>=Date.now():!0})}async function Se(){let o=[];try{o=await He()}catch(e){console.error("Offers API-\u0430\u0430\u0441 \u0430\u0432\u0430\u0445\u0430\u0434 \u0430\u043B\u0434\u0430\u0430:",e),o=[]}o=Pe(o),o=Be(o),localStorage.setItem("offers",JSON.stringify(o)),document.querySelectorAll("offers-list").forEach(e=>{e.items=o})}var K=class extends HTMLElement{connectedCallback(){this.render(),this.elements(),this.onClick=e=>this.handleClick(e),this.onUpdated=()=>this.load(),this.onHashChange=()=>this.load(),this.onMediaChange=()=>this.onViewportChange(),this.addEventListener("click",this.onClick),window.addEventListener("delivery-cart-updated",this.onUpdated),window.addEventListener("hashchange",this.onHashChange),this.media=window.matchMedia("(max-width: 54rem)"),this.media.addEventListener("change",this.onMediaChange),this.onViewportChange(),this.load()}disconnectedCallback(){this.removeEventListener("click",this.onClick),window.removeEventListener("delivery-cart-updated",this.onUpdated),window.removeEventListener("hashchange",this.onHashChange),this.media&&this.media.removeEventListener("change",this.onMediaChange)}render(){this.innerHTML=`
    ...
    `}elements(){this.listEl=this.querySelector(".delivery-cart__list"),this.emptyEl=this.querySelector(".delivery-cart__empty"),this.countEl=this.querySelector(".delivery-cart__count"),this.goBtn=this.querySelector(".delivery-cart__go")}load(){if((location.hash||"#home")==="#home"){if(this.isMobile){this.hideCart();return}this.fetchItems()}}async fetchItems(){try{let e=await u("/api/delivery-cart");if(!e.ok)this.items=[];else{let t=await e.json();this.items=Array.isArray(t?.items)?t.items:[]}}catch{this.items=[]}this.renderList()}renderList(){if(!this.listEl)return;let e=Array.isArray(this.items)?this.items:[];if(this.listEl.innerHTML="",!e.length){this.hideCart();return}this.showCart();let t=0;e.forEach(r=>{let i=Number(r.qty||1);t+=i;let s=ve(r.price),a=this.buildSubText(r.sub),n=document.createElement("div");n.className="delivery-cart__item",n.dataset.id=r.id||"",n.innerHTML=`
      ...
      `,this.listEl.appendChild(n)}),this.countEl&&(this.countEl.textContent=String(t))}buildSubText(e){let r=(Array.isArray(e)?e:[]).map(i=>i?.name).filter(Boolean);return r.length?r.join(", "):"\u0411\u0430\u0440\u0430\u0430 \u0430\u043B\u0433\u0430"}handleClick(e){if(e.target.closest(".delivery-cart__go")){location.hash="#delivery";return}let t=e.target.closest("[data-action='remove']");if(!t)return;let i=t.closest(".delivery-cart__item")?.dataset?.id;i&&this.decrementItem(i)}onViewportChange(){this.isMobile=this.media?.matches===!0,this.load()}async decrementItem(e){try{if(!(await fetch(`/api/delivery-cart/${e}`,{method:"PATCH"})).ok)return}catch{return}this.load(),window.dispatchEvent(new Event("delivery-cart-updated"))}showCart(){this.style.display="block";let e=this.closest(".offers-layout");e&&e.classList.add("has-cart"),this.emptyEl&&(this.emptyEl.style.display="none"),this.goBtn&&(this.goBtn.style.display="inline-flex")}hideCart(){this.style.display="none";let e=this.closest(".offers-layout");e&&e.classList.remove("has-cart"),this.emptyEl&&(this.emptyEl.style.display="block"),this.countEl&&(this.countEl.textContent="0"),this.goBtn&&(this.goBtn.style.display="none")}};customElements.define("delivery-cart",K);var G=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.elements(),this.events()}render(){this.shadowRoot.innerHTML=`
      <style>
        :host {
          --accent: #c90d30;
          --radius: 14px;
