(()=>{var R=class extends HTMLElement{connectedCallback(){this.onHashChange=()=>this.updateActive(),this.onUserUpdated=()=>this.handleUserUpdated(),this.onAppStateChanged=()=>this.handleAppStateChanged(),this.onDocClick=e=>this.handleDocClick(e),this.render(),window.addEventListener("hashchange",this.onHashChange),window.addEventListener("user-updated",this.onUserUpdated),window.addEventListener("app-state-changed",this.onAppStateChanged),this.loadUser(),this.updateActive()}disconnectedCallback(){window.removeEventListener("hashchange",this.onHashChange),window.removeEventListener("user-updated",this.onUserUpdated),window.removeEventListener("app-state-changed",this.onAppStateChanged),document.removeEventListener("click",this.onDocClick)}getAppState(){let e=localStorage.getItem("appState");if(e==="customer"||e==="courier")return e;let t=localStorage.getItem("authRole");return t==="customer"||t==="courier"?t:"guest"}getNavLinks(){let e=this.getAppState(),t=[];return e==="customer"&&(t.push({href:"#home",label:"\u041D\u04AF\u04AF\u0440",icon:"home"}),t.push({href:"#orders",label:"\u0417\u0430\u0445\u0438\u0430\u043B\u0433\u0430",icon:"orders"})),e==="courier"&&(t.push({href:"#home",label:"\u041D\u04AF\u04AF\u0440",icon:"home"}),t.push({href:"#delivery",label:"\u0425\u04AF\u0440\u0433\u044D\u043B\u0442",icon:"delivery"})),e!=="guest"&&t.push({href:"#profile",label:"\u041F\u0440\u043E\u0444\u0430\u0439\u043B",icon:"profile"}),t}renderNavLinks(){return this.getNavLinks().map(t=>`
      <a href="${t.href}">
        ${this.getIcon(t.icon)}
        <span>${t.label}</span>
      </a>
    ...
    `}applyActiveOrder(){this.fetchActiveOrder()}async fetchActiveOrder(){try{let e=await fetch("/api/active-order");if(!e.ok)return;let r=(await e.json())?.order;if(!r)return;let i=this.querySelector('d-orders[data-active="true"]');i&&r.from&&r.to&&(i.setAttribute("header",`${r.from} \u2192 ${r.to}`),r.item&&i.setAttribute("detail",r.item))}catch{}}renderDeliveryCart(){let e=this.querySelector("#deliveryList");e&&this.fetchDeliveryItems(e)}attachOrderSelection(e=[]){let t=this.querySelectorAll("d-orders");if(!t.length)return;let r=new Map(e.map(o=>[String(o.id||""),o]));t.forEach(o=>{o.addEventListener("click",()=>{let d=o.getAttribute("data-id"),c=r.get(String(d))||null;this.selectOrder(c,o)})});let i=t[0],s=i?.getAttribute("data-id"),a=r.get(String(s))||null;a&&this.selectOrder(a,i)}selectOrder(e,t){if(!e)return;this.querySelectorAll("d-orders").forEach(i=>{i.classList.toggle("is-active",i===t)});let r=new CustomEvent("delivery-select",{detail:{...e,id:e.orderId||e.id||null,orderId:e.orderId||null}});document.dispatchEvent(r)}async fetchDeliveryItems(e){let t=[];try{let a=await fetch("/api/delivery-cart");if(a.ok){let o=await a.json();t=Array.isArray(o?.items)?o.items:[]}}catch{t=[]}if(!t.length){e.innerHTML='<p class="muted">\u0425\u04AF\u0440\u0433\u044D\u043B\u0442 \u0441\u043E\u043D\u0433\u043E\u043E\u0433\u04AF\u0439 \u0431\u0430\u0439\u043D\u0430.</p>';let a=this.querySelector("del-order-details"),o=this.querySelector("del-order-progress");a&&a.setAttribute("data-empty","1"),o&&o.setAttribute("data-empty","1");return}let r=this.querySelector("del-order-details"),i=this.querySelector("del-order-progress");r&&r.removeAttribute("data-empty"),i&&i.removeAttribute("data-empty");let s=t.map(a=>{let o=Number(a.qty||1),d=[];a.meta&&d.push(a.meta),o&&d.push(`x${o}`);let c=d.join(" \u2022 "),l=String(a.title||""),h=l.split(" - "),u=h[0]||l,p=h.slice(1).join(" - "),w=a.order_id||null;return{...a,detail:c,from:u,to:p,createdAt:a.meta||"",orderId:w}});e.innerHTML=s.map(a=>`
        <d-orders
          data-id="${f(a.id||"")}"
          data-order-id="${f(a.orderId||"")}"
          data-from="${f(a.from||"")}"
          data-to="${f(a.to||"")}"
          data-created-at="${f(a.createdAt||"")}"
          header="${f(a.title||"")}"
          detail="${f(a.detail||"")}">
        </d-orders>
      `).join(""),this.attachOrderSelection(s)}};customElements.define("delivery-page",N);function C(n){return String(n??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/\"/g,"&quot;").replace(/'/g,"&#39;")}function x(n){return n>5?"assets/img/box.svg":n>=2?"assets/img/tor.svg":"assets/img/document.svg"}var D=class extends HTMLElement{connectedCallback(){this.render()}render(){this.innerHTML=`
      <div class="order-list" id="orderList">
          <p class="muted">\u0417\u0430\u0445\u0438\u0430\u043B\u0433\u0430 \u0430\u0447\u0430\u0430\u043B\u043B\u0430\u0436 \u0431\u0430\u0439\u043D\u0430...</p>
        </div>
    `}showLoading(){let e=this.querySelector("#orderList");e&&(e.innerHTML='<p class="muted">\u0417\u0430\u0445\u0438\u0430\u043B\u0433\u0430 \u0430\u0447\u0430\u0430\u043B\u043B\u0430\u0436 \u0431\u0430\u0439\u043D\u0430...</p>')}showEmpty(){let e=this.querySelector("#orderList");e&&(e.innerHTML='<p class="muted">\u0417\u0430\u0445\u0438\u0430\u043B\u0433\u0430 \u0430\u043B\u0433\u0430.</p>')}renderOrders(e){let t=this.querySelector("#orderList");if(t){if(!e.length){this.showEmpty();return}t.innerHTML=e.map(r=>this.renderOrderCard(r)).join("")}}renderOrderCard(e){let t=this.parseOrderTimestamp(e)||Date.now(),r=L(t),i=e.items,s=i.map(d=>`${d.name} \xD7${d.qty}`).join(" \xB7 "),a=i.reduce((d,c)=>d+(Number(c?.qty)||0),0),o=this.getOfferThumb(e?.id)||x(a);return`
    ...
    `;let c=this.querySelector(".btn-close"),l=this.querySelector(".btn-pay"),h=this.querySelector(".btn-go"),u=this.querySelector(".status"),p=this.querySelectorAll('input[name="paymentMethod"]'),w=()=>{u&&(u.hidden=!1),l&&(l.disabled=!0,l.textContent="\u0422\u04E9\u043B\u0431\u04E9\u0440 \u0442\u04E9\u043B\u04E9\u0433\u0434\u0441\u04E9\u043D"),h&&(h.hidden=!1)},I=()=>{u&&(u.hidden=!0),l&&(l.disabled=!1,l.textContent="\u0422\u04E9\u043B\u0431\u04E9\u0440 \u0442\u04E9\u043B\u04E9\u0445"),h&&(h.hidden=!0)},S=()=>{let g=this.querySelector('input[name="paymentMethod"]:checked')?.value||"card",v=this.querySelector(".payment-details--card"),y=this.querySelector(".payment-details--qpay");v&&(v.hidden=g!=="card"),y&&(y.hidden=g!=="qpay")};c?.addEventListener("click",()=>location.hash="#profile"),p.forEach(g=>g.addEventListener("change",S)),S(),localStorage.setItem("courierPaid",d()?"1":"0"),d()?w():I(),l?.addEventListener("click",()=>{localStorage.setItem(o,"1"),localStorage.setItem("courierPaid","1"),w(),window.dispatchEvent(new Event("user-updated"))}),h?.addEventListener("click",()=>{location.hash="#delivery"})}};customElements.define("pay-page",j);var F=class extends HTMLElement{connectedCallback(){this.currentMode="login",this.currentRole="customer",this.innerHTML=`
  ...
  <div class="scene">
    ...
    `;let e=this.querySelector("form"),t=this.querySelector(".close"),r=this.querySelector("input[name='role']"),i=this.querySelectorAll(".auth-tabs .tab-btn"),s=this.querySelectorAll(".login-tabs .tab-btn"),a=this.querySelector(".login-tabs"),o=this.querySelector("#login-title"),d=this.querySelector(".continue-btn"),c=this.querySelector(".subtitle"),l=this.querySelector("#name"),h=this.querySelector("#phone"),u=this.querySelector("#studentId"),p=this.querySelectorAll(".register-only");t&&t.addEventListener("click",()=>{location.hash="#home"}),i.forEach(S=>{S.addEventListener("click",()=>{let g=S.getAttribute("data-mode")||"login";this.currentMode=g,i.forEach(y=>{let M=y===S;y.classList.toggle("is-active",M),y.setAttribute("aria-selected",M?"true":"false")});let v=g==="register";p.forEach(y=>{y.style.display=v?"":"none"}),v&&r&&(r.value=this.currentRole),l&&(l.required=v),u&&(u.required=!1),h&&(h.required=!0),o&&(o.textContent=v?"\u0411\u04AF\u0440\u0442\u0433\u04AF\u04AF\u043B\u044D\u0445":"\u041D\u044D\u0432\u0442\u0440\u044D\u0445"),d&&(d.textContent=v?this.currentRole==="courier"?"\u0425\u04AF\u0440\u0433\u044D\u0433\u0447\u044D\u044D\u0440 \u0431\u04AF\u0440\u0442\u0433\u04AF\u04AF\u043B\u044D\u0445":"\u0425\u044D\u0440\u044D\u0433\u043B\u044D\u0433\u0447\u044D\u044D\u0440 \u0431\u04AF\u0440\u0442\u0433\u04AF\u04AF\u043B\u044D\u0445":"\u041D\u044D\u0432\u0442\u0440\u044D\u0445"),c&&(c.textContent=v?"\u041D\u044D\u0440 \u0431\u043E\u043B\u043E\u043D \u043D\u0443\u0443\u0446 \u04AF\u0433\u044D\u044D \u0430\u0448\u0438\u0433\u043B\u0430\u043D \u043D\u044D\u0432\u0442\u044D\u0440\u043D\u044D.":"\u0423\u0442\u0430\u0441\u043D\u044B \u0434\u0443\u0433\u0430\u0430\u0440, \u043D\u0443\u0443\u0446 \u04AF\u0433\u044D\u044D \u043E\u0440\u0443\u0443\u043B\u043D\u0430 \u0443\u0443.")})}),s.forEach(S=>{S.addEventListener("click",()=>{let g=S.getAttribute("data-role")||"customer";this.currentRole=g,a&&(a.dataset.activeRole=g),s.forEach(v=>{let y=v===S;v.classList.toggle("is-active",y),v.setAttribute("aria-selected",y?"true":"false")}),r&&(r.value=g),o&&(o.textContent="\u0411\u04AF\u0440\u0442\u0433\u04AF\u04AF\u043B\u044D\u0445"),d&&(d.textContent=g==="courier"?"\u0425\u04AF\u0440\u0433\u044D\u0433\u0447\u044D\u044D\u0440 \u0431\u04AF\u0440\u0442\u0433\u04AF\u04AF\u043B\u044D\u0445":"\u0425\u044D\u0440\u044D\u0433\u043B\u044D\u0433\u0447\u044D\u044D\u0440 \u0431\u04AF\u0440\u0442\u0433\u04AF\u04AF\u043B\u044D\u0445")})});let w=localStorage.getItem("login_prefill_mode"),I=localStorage.getItem("login_prefill_role");w&&(this.currentMode=w==="register"?"register":"login",localStorage.removeItem("login_prefill_mode")),I&&(this.currentRole=I==="courier"?"courier":"customer",localStorage.removeItem("login_prefill_role")),p.length&&p.forEach(S=>{S.style.display=this.currentMode==="register"?"":"none"}),a&&(a.dataset.activeRole=this.currentRole),r&&(r.value=this.currentRole),o&&(o.textContent=this.currentMode==="register"?"\u0411\u04AF\u0440\u0442\u0433\u04AF\u04AF\u043B\u044D\u0445":"\u041D\u044D\u0432\u0442\u0440\u044D\u0445"),d&&(d.textContent=this.currentMode==="register"?this.currentRole==="courier"?"\u0425\u04AF\u0440\u0433\u044D\u0433\u0447\u044D\u044D\u0440 \u0431\u04AF\u0440\u0442\u0433\u04AF\u04AF\u043B\u044D\u0445":"\u0425\u044D\u0440\u044D\u0433\u043B\u044D\u0433\u0447\u044D\u044D\u0440 \u0431\u04AF\u0440\u0442\u0433\u04AF\u04AF\u043B\u044D\u0445":"\u041D\u044D\u0432\u0442\u0440\u044D\u0445"),c&&(c.textContent=this.currentMode==="register"?"\u041D\u044D\u0440 \u0431\u043E\u043B\u043E\u043D \u043D\u0443\u0443\u0446 \u04AF\u0433\u044D\u044D \u0430\u0448\u0438\u0433\u043B\u0430\u043D \u043D\u044D\u0432\u0442\u044D\u0440\u043D\u044D.":"\u0423\u0442\u0430\u0441\u043D\u044B \u0434\u0443\u0433\u0430\u0430\u0440, \u043D\u0443\u0443\u0446 \u04AF\u0433\u044D\u044D \u043E\u0440\u0443\u0443\u043B\u043D\u0430 \u0443\u0443."),l&&(l.required=this.currentMode==="register"),e&&e.addEventListener("submit",async S=>{S.preventDefault();let g=this.querySelector("#password")?.value?.trim()||"",v=this.currentMode==="register",y=v&&(r?.value||"customer")==="courier",M=this.querySelector("#name")?.value?.trim()||"",oe=this.querySelector("#phone")?.value?.trim()||"",ne=this.querySelector("#studentId")?.value?.trim()||"",de=r?.value||this.currentRole||"customer",q=v&&de==="courier"?"courier":"customer",xe=M.trim()||"\u0417\u043E\u0447\u0438\u043D \u0445\u044D\u0440\u044D\u0433\u043B\u044D\u0433\u0447";try{let O=await m("/api/auth/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:xe,phone:oe,studentId:ne,role:q,password:g,mode:this.currentMode})});if(!O.ok){let $=await O.json().catch(()=>({}));throw new Error($.error||"\u041D\u044D\u0432\u0442\u0440\u044D\u0445 \u04AF\u0435\u0434 \u0430\u043B\u0434\u0430\u0430 \u0433\u0430\u0440\u043B\u0430\u0430")}let E=await O.json(),k=(E?.user?.role||E?.role||de||q||"customer")==="courier"?"courier":"customer",le=String(E?.user?.phone||oe||"").trim(),ce=String(E?.user?.student_id||E?.user?.studentId||ne||"").trim(),T=(ce||le).trim();if(localStorage.setItem("authLoggedIn","1"),localStorage.setItem("authRole",k),localStorage.setItem("authPhone",le),localStorage.setItem("authStudentId",ce),localStorage.setItem("authUserKey",T),k==="courier"&&T){let $=`courierPaid:${T}`,ke=localStorage.getItem($)==="1"?"1":"0";localStorage.setItem("courierPaid",ke)}else localStorage.setItem("courierPaid","0");q==="courier"?localStorage.getItem("courierPaid")!=="1"&&localStorage.setItem("courierPaid","0"):localStorage.setItem("courierPaid","0"),window.dispatchEvent(new Event("user-updated"));let Oe=localStorage.getItem("pendingOrderDraft");location.hash=Oe?"#home":"#profile";return}catch(O){let E=String(O?.message||"");if(E.includes("users_phone_key")){alert("\u042D\u043D\u044D \u0443\u0442\u0430\u0441 \u0431\u04AF\u0440\u0442\u0433\u044D\u043B\u0442\u044D\u0439 \u0431\u0430\u0439\u043D\u0430. \u041D\u044D\u0432\u0442\u0440\u044D\u0445 \u0433\u043E\u0440\u0438\u043C\u043E\u043E\u0440 \u043E\u0440\u043D\u043E \u0443\u0443.");let k=this.querySelector('.auth-tabs .tab-btn[data-mode="login"]');k&&k.click();return}alert(E||"\u041D\u044D\u0432\u0442\u0440\u044D\u0445 \u04AF\u0435\u0434 \u0430\u043B\u0434\u0430\u0430 \u0433\u0430\u0440\u043B\u0430\u0430")}})}normalizeName(e){let t=String(e||"").trim();if(!t)return"\u0417\u043E\u0447\u0438\u043D \u0445\u044D\u0440\u044D\u0433\u043B\u044D\u0433\u0447";let r=t.split(/\s+/).filter(i=>i&&i.length>1);return r.length?r.join(" "):t}};customElements.define("login-page",F);var J=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._timer=null}connectedCallback(){this.render(),this.updateToNow(),this.startAutoUpdate()}disconnectedCallback(){clearInterval(this._timer)}render(){this.shadowRoot.innerHTML=`
      <style>
        :host {
          display: flex;
          gap: 0.5rem;
    ...
    `}updateToNow(){let e=this.shadowRoot.querySelector(".date"),t=this.shadowRoot.querySelector(".time"),r=new Date,i=r.getFullYear(),s=String(r.getMonth()+1).padStart(2,"0"),a=String(r.getDate()).padStart(2,"0"),o=String(r.getHours()).padStart(2,"0"),d=String(r.getMinutes()).padStart(2,"0");e.value=`${i}-${s}-${a}`,t.value=`${o}:${d}`}startAutoUpdate(){this._timer=setInterval(()=>{this.updateToNow()},1e3*60)}get value(){let e=this.shadowRoot.querySelector(".date").value,t=this.shadowRoot.querySelector(".time").value;return`${e}\u2022${t}`}get iso(){let e=this.shadowRoot.querySelector(".date").value,t=this.shadowRoot.querySelector(".time").value,r=new Date(`${e}T${t}:00`);return isNaN(r.getTime())?null:r.toISOString()}};customElements.define("date-time-picker",J);function ye(n){return n>5?1500:n>=2?1e3:n===1?500:0}var z=class extends HTMLElement{constructor(){super()}connectedCallback(){this.render(),this.getElements(),this.setupEvents(),this.updateCartTotals()}render(){this.innerHTML=`
       <section>
        ...
        `}getElements(){if(this.totalPriceEl=this.querySelector(".total-price"),this.deliveryPriceEl=this.querySelector(".delivery-box p:last-child"),this.cartItemsContainer=this.querySelector(".cart-content div:first-child"),this.cartBadge=this.querySelector(".cart-icon span"),this.foodSelect=document.querySelector("#what")||document.querySelector(".bottom-row select"),this.deliveryImgEl=this.querySelector(".delivery-box img"),!this.cartItemsContainer){let e=this.querySelector(".cart-content");if(e){let t=document.createElement("div");e.insertBefore(t,e.firstChild),this.cartItemsContainer=t}}}setupEvents(){this.foodSelect&&this.foodSelect.addEventListener("change",e=>{let t=e.target,r=t.selectedOptions;if(r.length===0)return;let i=r[0],s="";i.dataset.name?s=i.dataset.name:s=i.textContent,s.indexOf(" \u2014 ")!==-1&&(s=s.split(" \u2014 ")[0]);let a=0;i.dataset.price&&(a=i.dataset.price);let o="";i.dataset.img&&(o=i.dataset.img),this.addItemToCart(s,a,o),t.selectedIndex=0}),this.cartItemsContainer&&this.cartItemsContainer.addEventListener("click",e=>{let t=e.target.closest("svg.del-btn");if(!t)return;let r=t.closest(".item-box");if(!r)return;let i=1;r.dataset.qty&&(i=parseInt(r.dataset.qty,10));let s=0;if(r.dataset.price&&(s=parseInt(r.dataset.price,10)),i>1){let a=i-1;r.dataset.qty=String(a);let o=r.querySelector("b"),d="";o&&(d=o.textContent||"");let c=r.querySelector("p");if(c){let h=C(d);c.innerHTML=`<b>${h}</b><br>${a} \u0448\u0438\u0440\u0445\u044D\u0433`}let l=r.querySelector(".price");if(l){let h=s*a;l.textContent=b(h)}}else r.remove();this.updateCartTotals()})}addItemToCart(e,t,r=""){let i=this.cartItemsContainer.querySelectorAll(".item-box"),s=null;for(let l=0;l<i.length;l++){let h=i[l],u=h.querySelector("b");if(!u)continue;if((u.textContent?u.textContent.trim():"")===e){s=h;break}}if(s){let l=1;s.dataset.qty&&(l=parseInt(s.dataset.qty,10));let h=l+1,u=t;if(s.dataset.price){let I=parseInt(s.dataset.price,10)}s.dataset.qty=String(h),s.dataset.price=String(u);let p=s.querySelector("p");if(p){let I=C(e);p.innerHTML=`<b>${I}</b><br>${h} \u0448\u0438\u0440\u0445\u044D\u0433`}let w=s.querySelector(".price");w&&(w.textContent=b(u*h)),this.updateCartTotals();return}let a=document.createElement("div");a.className="item-box",a.dataset.qty="1",a.dataset.price=String(t);let o="";if(r!==""){let l=f(r),h=f(e);o=`<img class="item-img" src="${l}" alt="${h}">`}let d=C(e),c=b(t);a.innerHTML=o+`<p><b>${d}</b><br>1 \u0448\u0438\u0440\u0445\u044D\u0433</p><p class="price">${c}</p><svg class="del-btn" viewBox="0 0 20 20" width="18" height="18" role="button" aria-label="remove">
            ...
            </svg>`,this.cartItemsContainer.appendChild(a),this.updateCartTotals()}updateCartTotals(){let e=[];this.cartItemsContainer&&(e=this.cartItemsContainer.querySelectorAll(".item-box"));let t=0,r=0,i=[];for(let c=0;c<e.length;c++){let l=e[c],h=1;l.dataset.qty&&(h=parseInt(l.dataset.qty,10));let u=0;l.dataset.price&&(u=parseInt(l.dataset.price,10));let p="",w=l.querySelector("b");w&&(p=(w.textContent||"").trim()),t=t+u*h,r=r+h,i.push({name:p,qty:h,price:u,unitPrice:u})}let s=ye(r),a="0\u20AE";t>0&&(a=b(s)),this.deliveryPriceEl&&(this.deliveryPriceEl.textContent=a);let o=0;t>0&&(o=t+s),this.totalPriceEl&&(this.totalPriceEl.textContent=b(o)),this.cartBadge&&(this.cartBadge.textContent=String(r));let d=x(r);this.deliveryImgEl&&(this.deliveryImgEl.src=d,this.deliveryImgEl.alt="delivery tier "+r),r===0?this.style.display="none":this.style.display="block",this.summary={items:i,itemsTotal:t,deliveryFee:s,total:o,totalQty:r,deliveryIcon:d}}parsePrice(e){return parseInt(String(e||"").replace(/[^\d]/g,""),10)}getSummary(){return this.summary}};window.customElements.define("sh-cart",z);var V=class extends HTMLElement{static get observedAttributes(){return["thumb","title","meta","price","sub","customer","order-id"]}connectedCallback(){this._ready||(this._ready=!0,this.render(),this.setupEvents())}attributeChangedCallback(){this._ready&&this.render()}get data(){return{thumb:this.getAttribute("thumb")||"assets/img/box.svg",title:this.getAttribute("title")||"",meta:this.getAttribute("meta")||"",price:this.getAttribute("price")||"",orderId:this.getAttribute("order-id")||"",sub:this.parseJSON(this.getAttribute("sub")||"[]"),customer:this.parseJSON(this.getAttribute("customer")||"{}")}}parseJSON(e){try{return JSON.parse(e)}catch{return e.startsWith("[")?[]:{}}}render(){let e=this.data,t=e.thumb,r=e.title,i=e.meta,s=e.price;this.innerHTML=`
    ...
    `}setupEvents(){this.addEventListener("click",()=>{this.sendSelectEvent()}),this.addEventListener("keydown",e=>{let t=e.key==="Enter",r=e.key===" ";(t||r)&&(e.preventDefault(),this.click())})}sendSelectEvent(){let e=this.data,t=new CustomEvent("offer-select",{bubbles:!0,detail:e});this.dispatchEvent(t)}};customElements.get("offer-card")||customElements.define("offer-card",V);var Z=class extends HTMLElement{connectedCallback(){this._ready||(this._ready=!0,this._loaded=!1,this.handleRouteChange=()=>{this.onRouteChange()},this.renderList(),this.handleOffersUpdated=()=>be(),window.addEventListener("offers-updated",this.handleOffersUpdated),window.addEventListener("hashchange",this.handleRouteChange),this.addEventListener("offer-select",this.handleOfferSelect),this.handleRouteChange())}disconnectedCallback(){this.handleOffersUpdated&&window.removeEventListener("offers-updated",this.handleOffersUpdated),window.removeEventListener("hashchange",this.handleRouteChange),this.removeEventListener("offer-select",this.handleOfferSelect)}renderList(){this.innerHTML=`
      <section class="offers-container">
        <div class="offers-row"></div>
      </section>
    `}set items(e){this.renderItems(e)}renderItems(e){let t=this.querySelector(".offers-row");t&&(t.innerHTML=e.map(qe).join(""))}handleOfferSelect(e){let t=document.querySelector("offer-modal");if(!t){console.warn("offer-modal \u043E\u043B\u0434\u0441\u043E\u043D\u0433\u04AF\u0439");return}t.show(e.detail)}onRouteChange(){let e=location.hash;if(e!=="#home"&&e!==""||this._loaded===!0)return;this._loaded=!0;let t=He();this.renderItems(t),be()}};customElements.get("offers-list")||customElements.define("offers-list",Z);function qe(n){let e=n.thumb||"assets/img/document.svg",t=n.title||"",r=n.meta||"",i=n.price||"",s=n.orderId||n.id||"",a=f(JSON.stringify(n.sub||[])),o=f(JSON.stringify(n.customer||{}));return`
    <offer-card
      thumb="${e}"
      title="${t}"
      meta="${r}"
  ...
  `}function we(n){try{let e=String(n).split("\u2022"),[t,r]=e,[i,s,a]=t.split("/"),[o,d]=r.split(":"),c=2e3+Number.parseInt(a,10);return new Date(c,Number(i)-1,Number(s),Number(o),Number(d))}catch{}}function Se(n){let e=n?.scheduled_at;if(!e)return null;let t=Date.parse(e);if(!isNaN(t))return t;let r=we(e);return r?r.getTime():null}function Te(n){let e=Se(n);return e?e<Date.now():!0}function $e(n){let e=n?.from_name||"",t=n?.to_name||"";return[e,t].filter(r=>r!=="").join(" - ")}function Re(n){return n.filter(e=>{let t=e?.status;return t==="delivered"||t==="cancelled"||e?.courier?!1:!Te(e)}).map(e=>{let t=e?.items||[],r=t.reduce((a,o)=>a+(o?.qty??0),0),i=t.map(a=>({name:`${a?.name||"\u0411\u0430\u0440\u0430\u0430"} x${a?.qty||1}`})),s=Se(e);return{orderId:e?.id||"",title:$e(e),meta:L(s),price:b(e?.total_amount),thumb:x(r),customer:e?.customer,sub:i}})}async function _e(){try{let n=await m("/api/orders");if(!n.ok)return console.error("\u0421\u0430\u043D\u0430\u043B\u0443\u0443\u0434 \u0442\u0430\u0442\u0430\u0445\u0430\u0434 \u0430\u043B\u0434\u0430\u0430:",n.status),[];let e=await n.json();return Re(e)}catch{}}function He(){let n=localStorage.getItem("offers");if(!n)return[];try{return JSON.parse(n)||[]}catch{}}function Ue(n){let e=localStorage.getItem("authUserKey");return e?`${n}:${e}`:n}function Ne(){let n=Ue("removed_offer_ids"),e=localStorage.getItem(n);if(!e)return[];try{return JSON.parse(e)||[]}catch{}}function De(n){let e=Ne();return e.length?n.filter(t=>{let r=t?.orderId??t?.id??null;return r?!e.includes(String(r)):!0}):n}function Be(n){return n.filter(e=>{if(!e?.meta)return!0;let t=we(e.meta);return t?t.getTime()>=Date.now():!0})}async function be(){let n=[];try{n=await _e()}catch{}n=Be(n),n=De(n),localStorage.setItem("offers",JSON.stringify(n)),document.querySelectorAll("offers-list").forEach(e=>{e.items=n})}var K=class extends HTMLElement{connectedCallback(){this.render(),this.getElements(),this.setupEventListeners(),this.checkViewportSize(),this.loadCartItems()}disconnectedCallback(){this.removeEventListeners()}render(){this.innerHTML=`
  ...
  <g id="Complete">
    ...
    `}};customElements.define("courier-card",ie);var se=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.value=0}connectedCallback(){let e=parseInt(this.getAttribute("max"))||5,t=this.getAttribute("color")||"gold",r=this.getAttribute("size")||"26px",i=document.createElement("style");i.textContent=`
      .rating {
        display: flex;
        gap: 5px;
        cursor: pointer;
